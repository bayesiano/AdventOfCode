...
object Day12b {
    ...
    private fun calculatePaths( cavern: String, map: List<Pair<String,String>>, path: Path = listOf()): List<Path> {
        ...
        if( cavern.isLowercase() && (path.count { cavern == it.second } > if( twice( path, cavern)) 1 else 2) )
            return listOf()
        return map.
    ...
    private fun twice( path: List<Pair<String, String>>, cavern: String) =
        path.map{ it.second}.filter { it != cavern && it.isLowercase()}.groupBy( {it }, {it}).any {
            it.value.size >= 2}
